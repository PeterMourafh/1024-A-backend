...
app.get("/pessoas", async (req, res) => {
    try {
        ...
        const mysqlErrorHandler = new MysqlErrorHandler(err, res)
        mysqlErrorHandler.validar()
    }
})
...
app.post("/cadastro_produto", async (req, res) => {
    ...
    try {
        const [result] =
            await connection
                .execute<ResultSetHeader>('INSERT INTO produto VALUES (?,?,?,?,?,?)', [id, nome, categoria, preco, data_criacao, data_modificacao])
        if (result.affectedRows === 0)
            return res.status(500).json({ mensagem: "Erro ao inserir!" })
        return res.status(201).json({ mensagem: "Sucesso ao inserir!" })
...
app.get("/listar_produtos_informatica", async (req, res) => {
    try {
        const [dados, campos] =
            await connection.execute<IProduto[]>('SELECT * FROM produto WHERE categoria = "informatica"')
        res.status(200).json(dados)
    } catch (err) {
        const mysqlErrorHandler = new MysqlErrorHandler(err, res)
        mysqlErrorHandler.validar()
